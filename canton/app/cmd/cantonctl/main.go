// Command cantonctl is the maintenance CLI for the NFL awards SQLite database:
...
func usage() {
	fmt.Fprint(os.Stderr, `cantonctl - maintain the NFL awards database

Usage:
  cantonctl import -db PATH JSON_FILE        Bulk-load award rows from a legacy JSON export
  cantonctl add -db PATH -year Y -player P -pos POS -unit U -team T -award A [-notes N]
                                           Add one validated award row
  cantonctl rm -db PATH -id ID               Remove a row by id
  cantonctl list -db PATH [-year Y] [-team T] [-award A] [-player P]
                                           List rows matching filters
  cantonctl export-json -db PATH OUT_FILE    Write the current dataset to a JSON file

Flags default -db to ../data/canton.db (run from the app/ directory).
...
func cmdImport(args []string) {
	...
	dbPath := fs.String("db", "../data/canton.db", "path to sqlite db")
	fs.Parse(args)
	if fs.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: cantonctl import -db PATH JSON_FILE")
		os.Exit(1)
	}
...
func cmdExport(args []string) {
	...
	fs.Parse(args)
	if fs.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: cantonctl export-json -db PATH OUT_FILE")
		os.Exit(1)
	}

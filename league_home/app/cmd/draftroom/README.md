...
hand too, and may have it open, so anything added since startup survives.
`cap` and `note` are left alone for the same reason — the board cannot set
them, so it must not erase them.

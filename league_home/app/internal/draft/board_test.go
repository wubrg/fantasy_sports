...
func TestFlagsForSharpDivergence(t *testing.T) {
	...
	if !has(PlayerSignals{SharpRankDelta: 16}, "sharp+") {
		t.Errorf("sharps ranking him higher should flag sharp+, got %v", flagsFor(PlayerSignals{SharpRankDelta: 16}))
	}
	if !has(PlayerSignals{SharpRankDelta: -18}, "sharp-") {
		t.Errorf("sharps ranking him lower should flag sharp-, got %v", flagsFor(PlayerSignals{SharpRankDelta: -18}))
	}
	// A move under the threshold is noise and must stay off the board.
	for _, f := range flagsFor(PlayerSignals{SharpRankDelta: 10}) {

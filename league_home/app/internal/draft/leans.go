...
// WriteLeans saves personal reads as a plain lean CSV.
//
...
// `leans -generate` eating hand-written reads. Writing mine.csv through it
// would either fail or, worse, succeed and mark the file generated, at which
// point the next generation would be free to destroy it.
//
...
func WriteLeans(path string, leans Leans) error {
	...
	tmp, err := os.CreateTemp(filepath.Dir(path), ".leans-*.csv")
	if err != nil {
		return fmt.Errorf("draft: writing leans: %w", err)
	}
